...
- **TorrentDetailParser (2025-11-02, RTC-53 follow-up)**: парсер теперь формирует доменную модель `Torrent` (см. `Remission/Domain/Torrent.swift`) и возвращает её через dependency `@Dependency(\.torrentDetailParser)`. `TorrentDetailReducer.State.apply(_:)` маппит доменную сущность в текущие поля состояния, а история скоростей добавляется отдельно. Благодаря этому переходу убран промежуточный `TorrentDetailParsedSnapshot`, и дальнейшая миграция UI сможет опираться на `Torrent` напрямую. Ошибки парсинга (`TorrentDetailParserError.missingTorrentData`) по-прежнему транслируются в `errorMessage` и покрыты тестом `detailsLoadedParserFailure`.
