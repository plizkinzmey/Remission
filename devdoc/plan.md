...
⚠️ **Дополнительные методы для мониторинга**:
- `session-stats` — агрегированная статистика сессии (активные торренты, скорости). Версия 3.0+. Используется для отображения глобальной статистики в UI.
- `torrent-set` — установка приоритетов, лимитов и других параметров для торрентов. Необходим для функции "Set Priority" в PRD.

Ниже остаётся краткая ориентировочная сводка (high-level), без детализации полей.
...
3. **HTTPS рекомендуется** для безопасности учетных данных. Приложение должно пытаться подключиться через HTTPS первым, затем предложить HTTP с предупреждением о безопасности (see Веха 2).
