...
**Последнее обновление:** 25 ноября 2025 г. (Архитектурная проверка Вехи 10, RTC-105)
...
- **Ключи**: кеш хранится per-server по UUID + fingerprint соединения (host/port/username/transport) + fingerprint учётных данных (SHA-256 от `accountIdentifier:password` без сохранения пароля). После handshake добавляется RPC-версия.
- **TTL**: 30 минут для слепков торрентов и сессии; просроченный кеш удаляется при чтении.
- **Размер**: лимит 5 МБ на сервер (JSON в Application Support/Remission/Snapshots); превышение лимита очищает кеш.
- **Инвалидация**: смена учётных данных, другая RPC-версия, удаление сервера, ошибка несовместимости или ошибка чтения/записи сбрасывают кеш.
- **Использование**: кеш читается при старте/офлайне, обновляется после успешных `torrent-get`/`session-get`, очищается при удалении сервера и при несовместимой версии Transmission.

2. Список торрентов
   - **Отображение**: каждая строка показывает имя, статус (иконка/текст), прогресс в % (и бар), текущие скорости `↓`/`↑`, ETA и счётчики пиров. Карточки обновляются без пересоздания при новых данных (анимации минимальные). Клики открывают детальный экран торрента.
   - **Фильтры**: сегментированный контрол с четырьмя состояниями — «Все», «Загрузки» (downloading/checking), «Раздачи» (seeding), «Ошибки» (status=isolated). Дополнительно доступен фильтр по категории (Все/Программы/Кино/Сериалы/Книги/Другое). Значения хранятся на уровне редьюсера и синхронизируются между запусками.
   - **Сортировка**: Picker с вариантами «Имя» (по алфавиту), «Прогресс» (от большего к меньшему), «Скорость» (downloadRate по убыванию), «ETA» (по возрастанию, незавершённые в конце). Пользователь видит активный порядок.
   - **Поиск**: `searchable` по подстроке имени торрента (case-insensitive). Пустой запрос показывает все элементы, изменение выполняется без повторного запроса к RPC (фильтрация по локальному списку).
   - **Пустые состояния**:
     - при `phase = loading` показываем placeholder/прогресс;
     - при `phase = loaded` и `items.isEmpty` — текст «Нет торрентов» + CTA «Добавить торрент»;
     - при ошибке (`phase = error`) показываем дружелюбное сообщение и кнопку «Повторить».
   - **Ошибки**: сетевые ошибки/ошибки настроек отображаются через TCA `AlertState` + баннер в списке. Пользователь может инициировать повтор.
  - **Обновление**: polling включён по умолчанию, интервал 5 секунд (можно изменить в настройках конкретного сервера). При ошибках применяется экспоненциальный backoff (1/2/4/8/16/30s). Manual pull-to-refresh запускает немедленный запрос. При `isAutoRefreshEnabled = false` polling выключается, но ручные обновления остаются доступными.
  - **Автоконтроль раздачи**: при включённом лимите рейтинга (seed ratio) приложение останавливает раздачу, если `uploadRatio > limit`, и запускает раздачу, если `uploadRatio < limit`, торрент завершён и находится в `stopped/seedWaiting`.
   - **UI-тесты/фикстуры**: для smoke-проверок используется launch-аргумент `--ui-testing-fixture=torrent-list-sample` + `--ui-testing-scenario=torrent-list-sample`, который подставляет три демонстрационные торрента (downloading/seeding/error) с предсказуемыми статусами.
...
4. Добавление торрентов
  ...
  - Единый экран: выбор источника (файл/магнет) + параметры добавления на той же форме (папка загрузки, start-paused, категория: Программы/Кино/Сериалы/Книги/Другое).
   - Каталог загрузки запоминает пользовательские пути и предлагает их в выпадающем списке с возможностью удалить кастомные значения (дефолтный путь всегда доступен).

5. Настройки
  - Настройки пер-сервер: polling-interval, автоматическое обновление при запуске, лимиты скорости по-умолчанию, лимит рейтинга раздачи (seed ratio) и телеметрия.
  - Управление сохранёнными серверами (add/edit/remove).

6. Логи и телеметрия
   - Единый логгер `AppLogger` (swift-log) доступен через `@Dependency(\.appLogger)`, категории создаются через `withCategory()`. Transmission RPC использует `DefaultTransmissionLogger` с маскированием Authorization/SessionID, усечением payload и контекстом (host/method/status/duration).
   - Локальное хранение логов: кольцевой буфер через `DiagnosticsLogStore` плюс файловый sink для prod-сборок (см. LOGGING_GUIDE). Доступ к свежим записям через экран «Диагностика» в Настройках (только локально, без отправки наружу).
  - Телеметрия — опт-ин: переключатель «Отправлять анонимную телеметрию» в настройках конкретного сервера, по умолчанию выключен. Все отправители обязаны проверять `TelemetryConsentDependency` перед отправкой; при `false` запросы не делаются.
   - Безопасность: пароли/магнеты/SessionID не пишутся в логи; Base64 и session-id маскируются, длинные ответы усечены. Гайд и пути к логам — `devdoc/LOGGING_GUIDE.md`.
...
- **Композиция TCA и эффекты**: новые фичи должны ссылаться на `devdoc/plan.md` раздел «Документация композиции редьюсеров и эффектов (RTC-59)», соблюдать `Scope`/`forEach`, `.ifLet` для presentation state и описанные правила `.run`/`.cancellation`.
...
- [ ] Новые фичи проходят `Templates/FeatureChecklist.md` и подтверждают соответствие гайдлайну RTC-59
...
**Политика выбора протокола (обновление RTC-41):**
- **HTTPS (рекомендуемый, значение по умолчанию)**  
  - При первом подключении переключатель протокола установлен на HTTPS.  
  - Успешное подключение не требует дополнительных подтверждений.  
  - Для сертификатов, выданных доверенным центром, соединение устанавливается без диалогов.  
  - При самоподписанном сертификате показываем системное предупреждение с иконкой `exclamationmark.triangle` и пояснением риска; текст должен чётко описывать, что сертификат не подтверждён. После подтверждения сохраняем SHA-256 отпечаток в Keychain и повторно спрашиваем при его изменении.
- **HTTP (доступен по выбору пользователя)**  
  - При переключении на HTTP показываем модальный диалог подтверждения с предустановленным фокусом на варианте "Отмена".  
  - Предупреждение обязательно содержит: риск передачи логина/пароля в открытом виде, рекомендацию использовать HTTPS и чекбокс "Не предупреждать для этого сервера".  
  - Решение "Не предупреждать" сохраняется в `UserDefaults` вместе с fingerprint конфигурации (host + port + username); при изменении параметров предупреждение показываем заново.
- **Self-signed / недоверенный сертификат**  
  - Первый показ — подробное окно с данными сертификата (CN, валидность, отпечаток) и вариантами "Доверять"/"Отмена".  
  - При выборе "Доверять" сохраняем SHA-256 отпечаток в Keychain и привязываем к связке host+port.  
  - При последующих подключениях, если отпечаток не совпадает или сертификат представлен с другим CN/организацией/промежуточными ЦА, считаем сертификат недоверенным, мгновенно удаляем сохранённый отпечаток и повторно показываем предупреждение.
- **Отключение предупреждений**  
  - Чекбокс "Не предупреждать" доступен только после разворота секции подробностей (DisclosureGroup) и требует явного согласия (клик по чекбоксу).  
  - Пользователь всегда может сбросить доверие/предупреждения в настройках сервера (см. таблицу ниже).

**Сценарные требования для UI:**

| Экран | Сценарий | Действие пользователя | Обязательное поведение | Хранение состояния |
| --- | --- | --- | --- | --- |
| Онбординг добавления сервера | Выбор HTTPS (по умолчанию) | Заполняет форму и нажимает "Проверить подключение" → после успеха "Сохранить сервер" | До успешной проверки кнопка "Сохранить сервер" неактивна, показываем текст "Проверка не выполнялась" / "Проверяем соединение…" / "Соединение подтверждено" | Нет |
| Онбординг добавления сервера | Переключение на HTTP | Подтверждает диалог | Показ модального предупреждения с описанием риска, кнопки "Продолжить" (role: destructive) и "Отмена"; при отмене возвращаем переключатель к HTTPS | Флаг "не предупреждать" (если выбран) в `UserDefaults` |
| Онбординг / Настройки сервера | HTTPS с self-signed | Подтверждает доверие | Диалог с деталями сертификата, кнопки "Доверять" и "Отмена", иконка предупреждения согласно HIG; при "Доверять" сохраняем отпечаток | Отпечаток в Keychain (привязка к серверу) |
| Настройки сервера | Сброс доверия/предупреждений | Нажимает "Сбросить" | Показываем подтверждение, очищаем соответствующие записи Keychain/UserDefaults, возвращаем переключатель к HTTPS | Удаляем отпечатки и флаги |
| Экран списка серверов | Сервер в режиме HTTP | Открывает строку | Отображаем предупреждающий бейдж (например, символ ⚠️) и подсказку о небезопасном соединении | Не требуется |
| Экран списка серверов | Сервер с self-signed после доверия | Нажимает "Сбросить доверие" | Кнопка в карточке сервера показывает Sheet подтверждения, после сброса бейдж ⚠️ снимается | Отпечаток удалён, флаги очищены |

**Тексты и локализация предупреждений:**
- HTTP предупреждение (RU): "Соединение без шифрования. Логин и пароль могут быть перехвачены. Продолжить?"  
  - EN: "Unencrypted connection. Username and password may be intercepted. Continue?"
- Self-signed предупреждение (RU): "Не удалось подтвердить сертификат сервера. Доверять этому сертификату?"  
  - EN: "Can't verify this server's certificate. Trust this certificate?"
- Чекбокс (RU/EN): "Не предупреждать для этого сервера" / "Don't warn again for this server".
- Статус проверки (RU): "Проверить подключение" (CTA), "Проверяем соединение…", "Соединение подтверждено", "Истекло время ожидания подключения…" (локализованное сообщение при timeout).

**UX-гайдлайны:**
- Используем системные стили предупреждений (`UIAlertController` / `NSAlert`) с иконкой `exclamationmark.triangle` только для повышенного риска согласно Apple HIG (см. [Human Interface Guidelines — Alerts](https://developer.apple.com/design/human-interface-guidelines/alerts)).  
- Сообщения формулируем коротко, избегаем технических терминов, объясняем риск понятным языком.  
- По умолчанию фокус на безопасном выборе: кнопка "Отмена" (`.cancel`) или "Не доверять".

**Диаграмма переходов состояния протокола (ускоренный reference для UX/QA):**
```
[HTTPS (default)]
      |
      | переключение HTTP
      v
[Диалог HTTP предупреждения] --(Отмена)--> [HTTPS (default)]
      |
      +--(Продолжить/флаг "Не предупреждать")--> [HTTP (подтвержден)]

[HTTPS (default)]
      |
      | самоподписанный сертификат
      v
[Диалог доверия сертификату] --(Отмена)--> [HTTPS (default)]
      |
      +--(Доверять)--> [HTTPS (доверенный self-signed)] --(изменение отпечатка/CN/CA)--> [Диалог доверия сертификату]

[HTTP (подтвержден)] --(Сброс в настройках)--> [HTTPS (default)]
```

**Синхронизация с политикой хранения данных:**
- Credentials (username/password) продолжают храниться только в Keychain (`kSecAttrService = com.remission`, `kSecAttrAccount = host:port:username`).  
- Отпечатки сертификатов (для self-signed) также сохраняем в Keychain как отдельные записи, чтобы не смешивать с паролями.  
- Флаги "не предупреждать" и выбранный протокол сохраняются в `UserDefaults` в структуре, не содержащей паролей. При сбросе сервера — очищаем обе области хранения.  
- Логи не содержат протокол, если он HTTP, и не пишут, что предупреждение отключено.

**Справочные материалы (RTC-41):**
- [Apple Human Interface Guidelines — Alerts](https://developer.apple.com/design/human-interface-guidelines/alerts) — рекомендации по оформлению критических предупреждений и использованию символов внимания.
...
- Подробный гайд для пользователей/поддержки: [LOGGING_GUIDE.md](LOGGING_GUIDE.md) (включение расширенного логирования, пути логов iOS/macOS, правила безопасности, экспорт/очистка).
- Телеметрия: переключатель в Settings → «Отправлять анонимную телеметрию», по умолчанию выключен; отправки только при явном согласии.
