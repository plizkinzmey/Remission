...
2. Список торрентов
   ...
   - **Фильтры**: сегментированный контрол с четырьмя состояниями — «Все», «Загрузки» (downloading/checking), «Раздачи» (seeding), «Ошибки» (status=isolated). Значение хранится на уровне редьюсера и синхронизируется между запусками.
