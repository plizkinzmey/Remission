...
### Передача данных
- **HTTPS обязателен**: при подключении к серверу всегда пытаться использовать HTTPS перед HTTP.
- **Проверка сертификатов**: валидировать SSL/TLS сертификаты сервера; при самоподписанных сертификатах — спросить пользователя перед продолжением.
- **Basic Auth**: отправлять учётные данные только через HTTPS; в HTTP-соединениях показать предупреждение.
