...
2. Список торрентов
   ...
   - **Обновление**: polling включён по умолчанию, интервал 5 секунд (можно изменить в «Настройки»). При ошибках применяется экспоненциальный backoff (1/2/4/8/16/30s). Manual pull-to-refresh запускает немедленный запрос. При `isAutoRefreshEnabled = false` polling выключается, но ручные обновления остаются доступными.
...
6. Логи и телеметрия
   ...
   - Телеметрия — опт-ин: переключатель «Отправлять анонимную телеметрию» в Настройках, по умолчанию выключен. Все отправители обязаны проверять `TelemetryConsentDependency` перед отправкой; при `false` запросы не делаются.
