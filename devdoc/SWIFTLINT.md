...
## Совместимость со swift-format

В проекте форматирование делает `swift-format`, а проверку стиля — SwiftLint. Конфигурации в целом согласованы (размер отступов, длина строки и т. п.), но есть тонкий момент:

- Правило SwiftLint `opening_brace` требует, чтобы фигурная скобка стояла на одной строке с объявлением (`if`, `struct`, `func`).
- `swift-format` с настройкой `respectsExistingLineBreaks: true` иногда переносит условие на несколько строк и может оставить скобку на следующей строке, если условие очень длинное.

### Как избежать конфликта

1. После форматирования проверяйте блоки `if`/`switch`/`struct` и переносите фигурную скобку обратно на строку объявления.
2. Если условие становится слишком громоздким, вынесите части в промежуточные `Bool` переменные или вспомогательные функции. Пример: `Remission/APIError.swift` использует флаги `containsParsingError` и `containsAuthError`, чтобы и линтер, и форматтер оставались довольны.
3. Всегда запускайте оба инструмента (`swift-format lint --strict …` и `swiftlint lint`) перед коммитом — так можно сразу отловить расхождения.
